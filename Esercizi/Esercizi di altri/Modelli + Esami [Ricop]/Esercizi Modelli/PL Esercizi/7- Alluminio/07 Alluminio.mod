...
# DATI
set Metalli;                        # Insieme dei metalli
set Materiali;                      # Insieme dei materiali
set Materiali2 within Materiali;    # Sottoinsieme dei materiali perche' sono gli unici 3 con limiti sulla quantita'
...
# VARIABILI
# Decidere le quantità ottimali di ogni materiale da miscelare
var x {i in Materiali} >=0;   # Quantità di materiale che viene impiegato per la fusione di alluminio 6063 [ton]

# VINCOLI
# Vincoli sulla max quantità disponibile di materiali [ton]
subject to Max_quantity {i in Materiali2}:
  x[i] <= q[i];

# La quantità complessiva prodotta deve essere pari alla domanda [ton]
...
minimize z: sum {m in Materiali} c[m] * x[m];
