...
var tot;
var caseificio{i in C} binary;

# VINCOLI

s.t. caseificioScelto:
	sum{i in C} caseificio[i] <= 1;


s.t. caseificioSceltoLatte{ i in C}:
	sum{j in A} AC[j,i] <= sum{j in A} P[j] * caseificio[i];
