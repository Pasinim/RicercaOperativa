# ESERCIZIO: PL Posologia

# DATI
set T := 1..24;          # Fasce orarie del giorno
param req {T};           # Quantità minima richiesta per ogni ora del giorno [mgY]
param maxliv;            # Quantità massima consentita [mgY]
...
# VARIABILI
var x {T,Farmaci} >=0;              # Quantità di ogni farmaco assunta in ogni ora del giorno [gX]
var y {t in T} >=req[t], <= maxliv; # Quantità di Y in ogni ora del giorno [mgY]

# VINCOLI
# Calcolo della quantità di Y in funzione delle quantità di X assunte in precedenza [mgY]
subject to Effetto {t2 in T}:
  y[t2] = sum {h in H, t1 in T, f in Farmaci: (t1 = t2-h) or (t1 = t2-h+24)} eff[h,f] * x[t1,f];

# OBIETTIVO
# Obiettivo n.1: Minimizzare la quantità totale di farmaco assunta [gX]
